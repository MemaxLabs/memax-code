...
func TestAppProgramCustomComposerFillsWithoutWritingFinalColumn(t *testing.T) {
	model := newAppProgramModel(context.Background(), options{CWD: "."}, nil)
	...
	for _, line := range strings.Split(raw, "\n") {
		if got := lipgloss.Width(line); got > 19 {
			t.Fatalf("custom composer line width = %d, want <= terminal width minus one:\n%q", got, raw)
		}
		if !strings.Contains(line, "\x1b[K") {
			t.Fatalf("custom composer line missing erase-to-end fill:\n%q", raw)
		}
	}
	if stripped := ansi.Strip(raw); strings.Contains(stripped, "\n›\n") || strings.Contains(stripped, "›\nAsk Memax Code") {
		t.Fatalf("custom composer split prompt from placeholder:\n%q", stripped)
	}
}

func TestAppProgramCustomComposerDoesNotTruncateShortInput(t *testing.T) {
	model := newAppProgramModel(context.Background(), options{CWD: "."}, nil)
	model.customRenderer = true
	...
	if !strings.Contains(stripped, "› Hello") {
		t.Fatalf("custom composer did not render short input with prompt:\n%q", stripped)
	}
	if strings.Contains(stripped, "...") {
		t.Fatalf("custom composer truncated a short input:\n%q", stripped)
	}
}

func TestAppProgramCustomRendererDropsTextareaBlinkCommand(t *testing.T) {
	...
	if cmd != nil {
		t.Fatalf("custom renderer returned textarea command %T, want nil to avoid idle repaint loop", cmd)
	}
	if !strings.Contains(ansi.Strip(out.String()), "› h") {
		t.Fatalf("custom renderer did not repaint typed input:\n%q", out.String())
	}
}

func TestAppProgramCustomRendererKeepsTextareaPasteCommand(t *testing.T) {
	var out bytes.Buffer
	model := newAppProgramModel(context.Background(), options{CWD: "."}, nil)
	model.output = &out
	model.customRenderer = true
	...
	if cmd == nil {
		t.Fatal("custom renderer dropped textarea paste command")
	}
}

func TestAppProgramCustomComposerWrapsLongInput(t *testing.T) {
	model := newAppProgramModel(context.Background(), options{CWD: "."}, nil)
	model.customRenderer = true
	model.input.SetValue("abcdefghijklmnopqrstuvwxyz")
	model.input.CursorEnd()

	raw := model.composerView(12)
	stripped := ansi.Strip(raw)
	for _, want := range []string{"› abcdefgh", "  ijklmnop", "  qrstuvwx", "  yz"} {
		if !strings.Contains(stripped, want) {
			t.Fatalf("custom composer did not soft-wrap long input row %q:\n%q", want, stripped)
		}
	}
	if strings.Contains(stripped, "...") {
		t.Fatalf("custom composer truncated long input instead of wrapping:\n%q", stripped)
	}
	if got, want := model.liveCursorRow, 5; got != want {
		t.Fatalf("custom composer cursor row = %d, want %d for wrapped input:\n%q", got, want, stripped)
	}
	if got, want := model.liveCursorCol, 6; got != want {
		t.Fatalf("custom composer cursor col = %d, want %d for wrapped input:\n%q", got, want, stripped)
	}
}

func TestAppProgramCustomComposerCursorFollowsTextareaSoftWrap(t *testing.T) {
	model := newAppProgramModel(context.Background(), options{CWD: "."}, nil)
	model.customRenderer = true
	model.width = 81
	model.height = 24
	model.resize()
	model.input.SetValue(strings.Repeat("a", 80))
	model.input.CursorEnd()

	raw := model.composerView(80)
	stripped := ansi.Strip(raw)
	if got, want := model.liveCursorRow, 3; got != want {
		t.Fatalf("custom composer cursor row = %d, want %d after textarea soft wrap:\n%q", got, want, stripped)
	}
	if got, want := model.liveCursorCol, 8; got != want {
		t.Fatalf("custom composer cursor col = %d, want %d after textarea soft wrap:\n%q", got, want, stripped)
	}
}

func TestAppProgramCustomRendererViewDoesNotPadAfterComposerFill(t *testing.T) {
	model := newAppProgramModel(context.Background(), options{
		CWD:   ".",
		Model: "anthropic/claude-sonnet-4-6-with-a-long-enough-name-to-fill-the-status-row",
	}, nil)
	model.customRenderer = true
	model.showHelp = true
	model.width = 80
	...
	for _, line := range strings.Split(view, "\n") {
		if got, want := ansi.StringWidth(line), appProgramLiveRegionWidth(model.width); got > want {
			t.Fatalf("custom view line width = %d, want <= %d:\n%q\nfull view:\n%q", got, want, line, view)
		}
	}
}
...
func TestAppProgramCustomRendererRepaintsFromBottomAnchor(t *testing.T) {
	var out bytes.Buffer
	model := newAppProgramModel(context.Background(), options{CWD: "."}, nil)
	model.output = &out
	model.customRenderer = true
	...
	model.renderLiveRegion()
	first := out.String()
	firstTop := model.liveRegionTop
	if firstTop <= 0 {
		t.Fatalf("first live render did not record live-region top row")
	}
	if !strings.Contains(first, appProgramMoveCursor(firstTop, 1)) {
		t.Fatalf("first live render did not move to bottom anchor:\n%q", first)
	}
	if strings.Contains(first, "\x1b[A") {
		t.Fatalf("first live render used relative row movement:\n%q", first)
	}
	if strings.Contains(first, "\x1b7") || strings.Contains(first, "\x1b8") {
		t.Fatalf("first live render used saved cursor anchoring:\n%q", first)
	}

	out.Reset()
	model.renderLiveRegion()
	second := out.String()
	secondTop := model.liveRegionTop
	for _, want := range []string{
		appProgramMoveCursor(secondTop, 1),
		appProgramClearToScreenEnd(),
		"Ask Memax Code",
	} {
		if !strings.Contains(second, want) {
			t.Fatalf("second live render missing %q:\n%q", want, second)
		}
	}
	if strings.Contains(second, "\x1b[A") {
		t.Fatalf("second live render used relative row movement:\n%q", second)
	}
	if strings.Contains(second, "\x1b7") || strings.Contains(second, "\x1b8") {
		t.Fatalf("second live render used saved cursor anchoring:\n%q", second)
	}
}

func TestAppProgramCustomRendererClearsPreviousTallerLiveRegion(t *testing.T) {
	var out bytes.Buffer
	model := newAppProgramModel(context.Background(), options{CWD: "."}, nil)
	model.output = &out
	model.customRenderer = true
	...
	model.resize()
	model.renderLiveRegion()
	oldTop := model.liveRegionTop
	if oldTop <= 0 {
		t.Fatalf("first render live-region top = %d, want positive", oldTop)
	}

	out.Reset()
	model.showHelp = false
	model.renderLiveRegion()
	newTop := model.liveRegionTop
	if newTop <= oldTop {
		t.Fatalf("shrunken live-region top = %d, want lower on screen than old top %d", newTop, oldTop)
	}
	got := out.String()
	if !strings.Contains(got, "\x1b[2J\x1b[H") {
		t.Fatalf("shrunken render did not force full replay before moving footer:\noldTop=%d newTop=%d\n%q", oldTop, newTop, got)
	}
	if !strings.Contains(got, appProgramMoveCursor(newTop, 1)) || !strings.Contains(got, "\x1b[48;5;235m\x1b[K") {
		t.Fatalf("shrunken render did not move back to new bottom anchor before drawing composer:\noldTop=%d newTop=%d\n%q", oldTop, newTop, got)
	}
}

func TestAppProgramCustomRendererReplaysTranscriptWhenFooterShrinksWithoutPrint(t *testing.T) {
	var out bytes.Buffer
	model := newAppProgramModel(context.Background(), options{CWD: "."}, nil)
	model.output = &out
	model.customRenderer = true
	...
	model.height = 12
	for i := 0; i < 20; i++ {
		model.appendLocalTranscriptLine("assistant", fmt.Sprintf("history-%02d", i))
	}
	...
	got := out.String()
	if newTop <= oldTop {
		t.Fatalf("shrunken live-region top = %d, want below old top %d", newTop, oldTop)
	}
	if !strings.Contains(got, "\x1b[2J\x1b[H") {
		t.Fatalf("footer shrink without print did not replay transcript:\n%q", got)
	}
	if !strings.Contains(got, "history-19") {
		t.Fatalf("footer shrink without print lost transcript tail:\n%q", got)
	}
}

func TestAppProgramCustomRendererReplaysTranscriptWhenFooterGrowsWithoutPrint(t *testing.T) {
	var out bytes.Buffer
	model := newAppProgramModel(context.Background(), options{CWD: "."}, nil)
	model.output = &out
	model.customRenderer = true
	...
	got := out.String()
	if newTop >= oldTop {
		t.Fatalf("grown live-region top = %d, want above old top %d", newTop, oldTop)
	}
	if !strings.Contains(got, "\x1b[2J\x1b[H") {
		t.Fatalf("footer growth without print did not replay transcript:\n%q", got)
	}
	if !strings.Contains(got, "history-19") {
		t.Fatalf("footer growth without print lost transcript tail:\n%q", got)
	}
}

func TestAppProgramCustomRendererClearsPreviousMultilineComposerRows(t *testing.T) {
	var out bytes.Buffer
	model := newAppProgramModel(context.Background(), options{CWD: "."}, nil)
	model.output = &out
	model.customRenderer = true
	...
	oldTop := model.liveRegionTop
	if oldTop <= 0 {
		t.Fatalf("multiline render live-region top = %d, want positive", oldTop)
	}
	...
	newTop := model.liveRegionTop
	if newTop <= oldTop {
		t.Fatalf("single-line live-region top = %d, want lower on screen than old top %d", newTop, oldTop)
	}
	got := out.String()
	if !strings.Contains(got, "\x1b[2J\x1b[H") {
		t.Fatalf("single-line render did not force full replay before moving footer:\noldTop=%d newTop=%d\n%q", oldTop, newTop, got)
	}
	if !strings.Contains(got, appProgramMoveCursor(newTop, 1)) || !strings.Contains(got, "\x1b[48;5;235m\x1b[K") {
		t.Fatalf("single-line render did not move back to new bottom anchor before drawing composer:\noldTop=%d newTop=%d\n%q", oldTop, newTop, got)
	}
}

func TestAppProgramCustomRendererTranscriptPrintClearsLiveRegion(t *testing.T) {
	var out bytes.Buffer
	model := newAppProgramModel(context.Background(), options{CWD: "."}, nil)
	model.output = &out
	model.customRenderer = true
	...
	model.resize()
	model.renderLiveRegion()
	liveTop := model.liveRegionTop

	out.Reset()
	model.printTranscriptLines([]string{"transcript line"})
	got := out.String()
	for _, want := range []string{
		appProgramMoveCursor(liveTop, 1),
		appProgramClearToScreenEnd(),
		appProgramScrollUp(1),
		appProgramMoveCursor(liveTop-1, 1),
		"transcript line",
	} {
		if !strings.Contains(got, want) {
			t.Fatalf("transcript print missing %q:\n%q", want, got)
		}
	}
	if model.liveRegion {
		t.Fatal("transcript print left live region marked active")
	}
	if model.liveRegionTop != 0 {
		t.Fatalf("transcript print left live region top = %d, want 0", model.liveRegionTop)
	}
	if model.liveRegionRows != 0 {
		t.Fatalf("transcript print left live region rows = %d, want 0", model.liveRegionRows)
	}
}

func TestAppProgramCustomRendererTranscriptMsgScrollsAboveLiveRegion(t *testing.T) {
	var out bytes.Buffer
	model := newAppProgramModel(context.Background(), options{CWD: "."}, nil)
	model.output = &out
	model.customRenderer = true
	...
	liveTop := model.liveRegionTop
	if liveTop <= 1 {
		t.Fatalf("live region top = %d, want scrollable transcript region", liveTop)
	}

	out.Reset()
	_, _ = model.Update(appProgramTranscriptMsg{text: "transcript-line-A\ntranscript-line-B\n"})
	got := out.String()
	for _, want := range []string{
		appProgramScrollUp(3),
		appProgramMoveCursor(liveTop-3, 1),
		"Welcome. Type a task or /help.\r\ntranscript-line-A\r\ntranscript-line-B",
	} {
		if !strings.Contains(got, want) {
			t.Fatalf("transcript update missing %q:\n%q", want, got)
		}
	}
	if strings.Contains(got, appProgramMoveCursor(liveTop, 1)+appProgramClearToScreenEnd()+"transcript-line-A") {
		t.Fatalf("transcript was printed inside the live footer region:\n%q", got)
	}
}

func TestAppProgramCustomRendererTranscriptPrintDoesNotDropLargeBatches(t *testing.T) {
	var out bytes.Buffer
	model := newAppProgramModel(context.Background(), options{CWD: "."}, nil)
	model.output = &out
	model.customRenderer = true
	...
	available := liveTop - 1
	if available < 2 {
		t.Fatalf("live region top = %d, want room for transcript batch test", liveTop)
	}

	lines := make([]string, 0, available+2)
	for i := 0; i < available+2; i++ {
		lines = append(lines, fmt.Sprintf("line-%02d", i))
	}
	out.Reset()
	model.printTranscriptLines(lines)
	got := out.String()
	for _, want := range []string{
		appProgramScrollUp(available),
		"line-00",
		fmt.Sprintf("line-%02d", available-1),
		appProgramScrollUp(2),
		fmt.Sprintf("line-%02d", available),
		fmt.Sprintf("line-%02d", available+1),
	} {
		if !strings.Contains(got, want) {
			t.Fatalf("large transcript batch missing %q:\n%q", want, got)
		}
	}
}

func TestAppProgramCustomRendererRedrawsTranscriptWhenFooterGrowsAfterPrint(t *testing.T) {
	var out bytes.Buffer
	model := newAppProgramModel(context.Background(), options{CWD: "."}, nil)
	model.output = &out
	model.customRenderer = true
	...
	model.resize()
	model.renderLiveRegion()
	idleTop := model.liveRegionTop
	if idleTop <= 0 {
		t.Fatalf("idle live-region top = %d, want positive", idleTop)
	}

	out.Reset()
	model.running = true
	model.statusLine = "running"
	model.appendLocalTranscriptLine("user", "› hello")
	_ = model.flushPrints()
	model.renderLiveRegion()
	got := out.String()
	if !strings.Contains(got, "\x1b[2J\x1b[H") {
		t.Fatalf("footer growth did not force full transcript redraw:\n%q", got)
	}
	if !strings.Contains(got, "› hello") {
		t.Fatalf("footer growth redraw lost freshly printed prompt:\n%q", got)
	}
	if model.liveRegionTop >= idleTop {
		t.Fatalf("running live-region top = %d, want above idle top %d", model.liveRegionTop, idleTop)
	}
}

func TestAppProgramCustomRendererRedrawsTranscriptWhenFooterShrinksAfterPrint(t *testing.T) {
	var out bytes.Buffer
	model := newAppProgramModel(context.Background(), options{CWD: "."}, nil)
	model.output = &out
	model.customRenderer = true
	...
	model.resize()
	model.renderLiveRegion()
	runningTop := model.liveRegionTop
	if runningTop <= 0 {
		t.Fatalf("running live-region top = %d, want positive", runningTop)
	}

	out.Reset()
	model.running = false
	model.statusLine = "idle"
	model.appendLocalTranscriptLine("assistant", "done")
	_ = model.flushPrints()
	model.renderLiveRegion()
	got := out.String()
	if !strings.Contains(got, "\x1b[2J\x1b[H") {
		t.Fatalf("footer shrink did not force full transcript redraw:\n%q", got)
	}
	if !strings.Contains(got, "done") {
		t.Fatalf("footer shrink redraw lost freshly printed assistant line:\n%q", got)
	}
	if model.liveRegionTop <= runningTop {
		t.Fatalf("idle live-region top = %d, want below running top %d", model.liveRegionTop, runningTop)
	}
}

func TestAppProgramCustomRendererWritesCRLFInRawMode(t *testing.T) {
	var out bytes.Buffer
	model := newAppProgramModel(context.Background(), options{CWD: "."}, nil)
	model.output = &out
	model.customRenderer = true
	...
	model.renderLiveRegion()
	got := out.String()
	if !strings.Contains(got, appProgramHideCursor()) {
		t.Fatalf("custom renderer did not hide the terminal cursor:\n%q", got)
	}
	if !strings.Contains(got, appProgramShowCursor()) {
		t.Fatalf("custom renderer did not restore the terminal cursor inside the composer:\n%q", got)
	}
	for i, r := range got {
		if r == '\n' && (i == 0 || got[i-1] != '\r') {
			t.Fatalf("custom renderer wrote bare LF in raw terminal mode:\n%q", got)
		}
	}
}

func TestAppProgramCustomTerminalCursorEscapes(t *testing.T) {
	if got, want := appProgramHideCursor(), "\x1b[?25l"; got != want {
		t.Fatalf("hide cursor escape = %q, want %q", got, want)
	}
	if got, want := appProgramShowCursor(), "\x1b[?25h"; got != want {
		t.Fatalf("show cursor escape = %q, want %q", got, want)
	}
	if got, want := appProgramRestoreCursorForShell(24), "\x1b[24;1H\x1b[J\x1b[?25h\r\n"; got != want {
		t.Fatalf("restore cursor escape = %q, want %q", got, want)
	}
}

func TestAppProgramCustomRendererDoesNotRepaintAfterIdleQuit(t *testing.T) {
	var out bytes.Buffer
	model := newAppProgramModel(context.Background(), options{CWD: "."}, nil)
	model.output = &out
	model.customRenderer = true
	...
	model.renderLiveRegion()
	liveTop := model.liveRegionTop
	clearPrefix := model.liveRegionClearPrefixFromRow(liveTop)
	...
	got := out.String()
	if !model.quitting {
		t.Fatal("ctrl+c idle did not mark app as quitting")
	}
	if !strings.Contains(got, "bye") {
		t.Fatalf("idle quit did not print bye:\n%q", got)
	}
	if strings.Contains(strings.TrimPrefix(got, clearPrefix), "Ask Memax Code") {
		t.Fatalf("idle quit repainted live footer after bye:\n%q", got)
	}
}

func TestAppProgramCustomRendererDoesNotRepaintAfterForceQuit(t *testing.T) {
	var out bytes.Buffer
	model := newAppProgramModel(context.Background(), options{CWD: "."}, nil)
	model.output = &out
	model.customRenderer = true
	...
	got := out.String()
	if !model.quitting {
		t.Fatal("force ctrl+c did not mark app as quitting")
	}
	if !strings.Contains(got, "force quit") {
		t.Fatalf("force quit did not print transcript line:\n%q", got)
	}
	if strings.Contains(strings.TrimPrefix(got, clearPrefix), "Ask Memax Code") {
		t.Fatalf("force quit repainted live footer after transcript line:\n%q", got)
	}
}
